...
{
    public class PaymentPaytmIpnController : Controller
    {
        #region Fields
        ...
        protected virtual async Task ProcessRecurringPaymentAsync(string invoiceId, PaymentStatus newPaymentStatus, string transactionId, string ipnInfo)
        {
            ...
            foreach (var rp in recurringPayments)
            {
                switch (newPaymentStatus)
                {
                    ...
                    case PaymentStatus.Voided:
                        ...
                        {
                            Errors = new[] { $"Paytm IPN. Recurring payment is {nameof(PaymentStatus.Voided).ToLowerInvariant()} ." },
            ...
            await _logger.InformationAsync("Paytm IPN. Recurring info", new NopException(ipnInfo));
        }
